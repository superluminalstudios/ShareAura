		...
		<ion-content>
			<transition name="fade">
				<div class="popup" v-show="blur">
					<div class="popup-top">
						<img :src="`https://ui-avatars.com/api/?background=random&name=${modalUsername.charAt(0)}`" class="user-avatar-small" />
						<h3 class="request-title">{{ modalUsername }}</h3>
					</div>
					<div class="popup-mid-wrapper">
						<div>
							<h4 class="popup-mid" v-for="item in modalItemList" :key="item.index">· {{ item }}</h4>
						</div>
						<div class="popup-bottom-wrapper">
							<div class="popup-bottom-unim" @click="closeModal">
								Cancel
							</div>
							<a target="_blank" :href="`https://api.whatsapp.com/send/?phone=%+968${modalUsernumber}&text=Hey, I've seen your post on ShareAura and I have the books you need. Let's share?`" @click="closeModal">
			...
			<div :class="{ filterBlurOn: blur }">
				...
				<div class="lower-body">
					...
					<div class="request-post" v-for="textbook in allReqList" :key="textbook._id" @click="openModal(textbook.username, textbook.usernumber, textbook.itemList)">
...
export default defineComponent({
	...
	setup() {
		...
		onMounted(() =>
			axios.get(`https://staiclientapi.jeswinsunsi.repl.co/${route.params.school.replace(":", "")}/category/textbook/15`).then((data) => {
		...
		function changeReqList(param) {
			axios.get(`https://staiclientapi.jeswinsunsi.repl.co/${route.params.school.replace(":", "")}/category/${param}/15`).then((data) => {
				for (let itemNum in data.data) {
					let value = data.data[itemNum];
                    allReqList.value.length = 0;
					allReqList.value.push(value);
				}
			});
		}
